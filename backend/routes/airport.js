...
// Packages model
let Packages = require('../models/Airport');

// POST Packages
router.post('/create-air', upload.single('avatar'), (req, res, next) => {
  ...
  user.save().then(result => {
    console.log(result);
    res.status(201).json({
      message: "Airport Taxi registered successfully!",
      Package: {
        _id: result._id,
        name: result.name,
...
router.get("/:id", (req, res, next) => {
    ...
    if (data) {
      ...
      res.status(404).json({
        message: "Airport taxi not found!"
      });
    }
  });
});
